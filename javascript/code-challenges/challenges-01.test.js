...
const allUpperCase = (arr) => {
  ...
  arr.forEach((i) => {
    let lowerString = i;
    let pushValue = lowerString.toUpperCase();
    newArray3.push(pushValue);
  });
  return newArray3;
};
...
const greeting = (word) => {
  let fullString = word.toUpperCase() + '!';
  return fullString;
};
...
const speaker = (words, callback) => {
  let newArray4 = [];
  words.forEach((word) => {
    let capitalized = callback(word);
    newArray4.push(capitalized);
  });
  return newArray4;
};
...
const createList = (availableItems) => {
  ...
  availableItems.forEach((i) => {
    let produce = availableItems[i].name
    if (availableItems[i].available === true){
      newArray.push(produce);
    }
    return newArray;
  });
};
